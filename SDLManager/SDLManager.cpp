...
// Date:		   4/4/2017
// Class:		   CS 250
// Purpose:		 Implement an SDL C++ interface
//******************************************************************************
	...
	const uint8_t fps)
{
	...
	loadFont(rcFontLocation, fontSize);
	/*
...
* Method:      playSound
*
* Description: Play a WAV soundfile
*
* Parameters:  rcSoundLocation - The location of the sound file to play
*              bLoop           - Whether or not the sound should be looped
*
* Returned:    None
******************************************************************************/
void SDLManager::playSound(const std::string &rcSoundLocation, bool bLoop)
{
	mpsAudioBuffer = Mix_LoadWAV_RW(SDL_RWFromFile(rcSoundLocation.c_str(),
		"rb"), 1);
	if (nullptr == mpsAudioBuffer)
	{
		throw "Unable to open audio with error" + std::string(Mix_GetError());
	}
	Mix_VolumeChunk(mpsAudioBuffer, MIX_MAX_VOLUME);

	if (bLoop)
	{
		Mix_PlayChannel(-1, mpsAudioBuffer, -1);
	}
	else
	{
		Mix_PlayChannel(-1, mpsAudioBuffer, 0);
	}
}


/*******************************************************************************
* Method:      soundIsPlaying
*
* Description: Determine whether the SDLManager is currently playing a sound
*
* Parameters:  None
*
* Returned:    True if a sound is currently playing, false otherwise.
******************************************************************************/
bool SDLManager::isSoundPlaying() const
{
	return mpsAudioBuffer != nullptr;
}

/*******************************************************************************
* Method:      setSoundPause
*
* Description: Set the pause state of the playing sound (ie true = paused)
*
* Parameters:  bState - The desired pause state as described
*
* Returned:    None
******************************************************************************/
void SDLManager::setSoundPause(bool bState)
{
	if (isSoundPlaying())
	{
		if (bState)
...
* Method:      stopSound
*
* Description: Stop the sound from playing, freeing the memory associated with
*              the old sound
*
* Parameters:  None
*
